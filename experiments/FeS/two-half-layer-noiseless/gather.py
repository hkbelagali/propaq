...
for path in RESULTS_DIR.glob("FeS_LUCJ_o*.npz"):
    ...
    try:
        ...
        raw[order][tid] = {
            ...
            "n_tasks": ntasks,
            "n_input_monomials": int(d["n_input_monomials"]) if "n_input_monomials" in d else None,
...
for order in sorted(raw, reverse=True):
    ...
    monomial_counts = [task_map[tid]["n_input_monomials"] for tid in present_ids
                       if task_map[tid]["n_input_monomials"] is not None]
    if monomial_counts:
        present_total = sum(monomial_counts)
        present_str = str(present_total)
        if not missing_ids:
            missing_str = "0"
        elif present_ids:
            missing_str = f"~{round(present_total / len(monomial_counts) * len(missing_ids))}"
        else:
            missing_str = "?"
    else:
        present_str = "--"
    ...
    if missing_ids:
        print(f"WARNING: order {order} — {present_str} terms present, {missing_str} missing")
    ...
    order_results[order] = (ev_sum, n_tasks, present_ids, missing_ids, rt_mean, rt_std,
                             present_str, missing_str, damping, coeff_cutoff)
...
for order in sorted(order_results, reverse=True):
    ev_sum, n_tasks, present_ids, missing_ids, rt_mean, rt_std, present_str, missing_str, damping, coeff_cutoff = order_results[order]
    cumulative += ev_sum
    ...
    if damping is not None or coeff_cutoff is not None:
        ...
        params = f"  [{', '.join(parts)}]"
    print(
        f"{order:>7}  {present_str:>{TW}}  {missing_str:>{TW}}  "
