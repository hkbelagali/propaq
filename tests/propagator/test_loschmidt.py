...
def test_loschmidt_echo(): 
    ...
    noise = UniformNoiseModel(damping=0.0) 
    ...
    for term, coeff in obs.items(): 
        ...
        assert np.isclose(coeff, recovered_coeff, atol=1e-6), f"Term {term} was not recovered correctly: {coeff.real} vs {recovered_coeff.real}"
