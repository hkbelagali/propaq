...
def _dm_expectation_noisy(qc: QuantumCircuit, obs: SparsePauliOp, damping: float) -> float:
    """Schrodinger-picture DensityMatrix simulation with per-layer per-qubit depolarizing noise."""
    n = qc.num_qubits
...
def test_noiseless_majorana_pauli_agree_with_statevector(seed, obs_str):
    ...
    maj_ev = (
        MajoranaPropagator(None, TRUNC)
        .expectation_value(maj_obs, mc, initial_state=0)
        .expectation_value
    )
    pau_ev = (
        PauliPropagator(None, TRUNC)
        .expectation_value(pau_obs, pc, initial_state=0)
        .expectation_value
    )
...
def test_noisy_majorana_pauli_agree(seed, obs_str):
    ...
    maj_ev = (
        MajoranaPropagator(noise, TRUNC)
        .expectation_value(maj_obs, mc, initial_state=0)
        .expectation_value
    )
    pau_ev = (
        PauliPropagator(noise, TRUNC)
        .expectation_value(pau_obs, pc, initial_state=0)
        .expectation_value
    )

    assert np.isclose(maj_ev, pau_ev, atol=1e-10), (
        f"Majorana vs Pauli (noisy): {maj_ev} vs {pau_ev}"
    )
