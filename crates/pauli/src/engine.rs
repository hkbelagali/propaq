///
/// The monoprop-shaped engine behind the Pauli propagator's Python API.
///
/// `PartitionedOperator` is monomorphized on its storage width, so reaching it
/// from a runtime qubit count needs a dispatch. That dispatch lives here, along
/// with the driver that walks a circuit's layers.
///
/// This is propaq's only numerical backend. A request wider than the dispatch
/// ladder is refused rather than served slowly; there is nothing to fall back
/// to.
///
use pyo3::prelude::*;
...
use crate::algebra::{from_monomial, to_monomial, PauliAlgebra};
use crate::string::PauliString;

/// Widest qubit count the dispatch covers.
///
/// Beyond this a request is refused with a `ValueError` naming this limit;
/// there is no second backend to fall back to. Adding a width is one
/// line, but every instantiation multiplies compiled code across both bases and
/// every coefficient type, so the ladder stops where the monomorphized surface
/// stops being worth it rather than at a physical limit. `u16` positions address
/// 65535 bits, so the position type is not the constraint here.
pub const MAX_DISPATCH_QUBITS: usize = 2048;
...
///
/// Heisenberg propagation consumes layers in reverse, and each layer's
/// rotations in reverse, which is the order the propagator uses. Matching it
/// exactly is what makes the two engines comparable on the same circuit.
fn extract_layers<C: CoeffRepr>(py: Python<'_>, circuit: &Bound<'_, PyAny>) -> PyResult<Vec<Vec<Gate<C>>>> {
    ...
    for layer in layers.iter().rev() {
        ...
        for rot_obj in layer.iter().rev() {
            ...
            let generator: PauliString = rot.getattr("generator")?.extract()?;
...
/// Runs one circuit at a fixed storage width.
fn run_at_width<C, const W: usize, P>(
    observable: &[(PauliString, f64)],
    layers: &[Vec<Gate<C>>],
    n_units: usize,
    partitions: usize,
    cutoff: &EmitCutoff,
...
{
    // Size the rows before the first term lands: the store cannot restride
    // later, and a row that outgrows its width pays an overflow-map lookup on
    // every read of it thereafter.
    ...
    for (term, coeff) in observable {
        ...
        if !cutoff.admits_initial::<PauliAlgebra, C, W>(&key, &c, n_units) {
            continue;
        }
    ...
    for (layer_idx, layer) in layers.iter().enumerate() {
        ...
        if let Some(table) = noise {
            op.scale_by_weight::<PauliAlgebra>(|w| table[(w as usize).min(table.len() - 1)]);
    ...
    let terms = want_terms.then(|| {
        op.iter::<PauliAlgebra>()
            .map(|(key, sign, c)| {
                let mut coeff = c.clone();
                coeff.scale_real(sign);
                (from_monomial::<W>(&key, n_units), coeff)
            })
            .collect::<Vec<_>>()
    });
    let expectation_value = fock.map_or(0.0, |f| op.expectation::<PauliAlgebra>(f));
...
/// Dispatches on qubit count to a monomorphized width, then runs the circuit.
///
/// Width is `ceil(2 * n_units / 64)`, since a monomial carries two bits per
/// qubit. The position type is the narrowest that can address that width's bits:
/// `u8` reaches 255 positions, which covers 128 bits but not 256.
macro_rules! dispatch_width {
    ($c:ty, $n_units:expr, $($limit:expr => ($w:expr, $pos:ty)),+ $(,)?) => {{
        let n = $n_units;
        $(if n <= $limit {
            return run_at_width::<$c, $w, $pos>;
        })+
        unreachable!("caller must check MAX_DISPATCH_QUBITS first")
    }};
}
...
pub struct RunOutput<C> {
    ...
    pub terms: Option<Vec<(PauliString, C)>>,
    /// Phase timings and kernel counters for the run's log.
    pub phases: PhaseStats,
}

type Runner<C> = fn(&[(PauliString, f64)], &[Vec<Gate<C>>], usize, usize, &EmitCutoff, Option<&NoiseTable>, bool, Option<&[u64]>, bool, bool)
    -> PyResult<RunOutput<C>>;

/// The monomorphized runner for this qubit count and coefficient width.
...
pub fn run<C: CoeffRepr>(
    ...
    let noise_table = resolve_noise_table(noise, n_units)?;
    ...
    let out = py.allow_threads(|| {
        pool.install(|| {
            run(observable, &layers, n_units, partitions, &cutoff, noise_table.as_ref(),
