...
use crate::algebra::{from_basis_string, to_basis_string, PauliAlgebra};
    ...
    P: propaq_core::operator_index::Pos,
{
    ...
    for (layer_idx, layer) in layers.iter().enumerate() {
        ...
        if let Some(noise) = noise {
            match noise {
                ...
                ResolvedNoise::PythonTerm(model) => Python::attach(|py| -> PyResult<()> {
                    ...
                    op.try_scale_by_key::<PauliAlgebra, PyErr>(|key, weight| {
                        model
                            .call_method1(
                                PYTHON_TERM_HOOK,
                                (
                                    BasisKind::Pauli.as_u32(),
                                    key.words().to_vec(),
                                    n_units,
                                    weight,
                                ),
        ...
        for gate in layer {
            ...
            let gained = op
                .apply_rotation::<PauliAlgebra>(&gen, &gate.angle, &cutoff)
                .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
            if let Some(t0) = t0 {
    ...
    let terms = want_terms.then(|| {
        op.iter::<PauliAlgebra>()
            .map(|(key, sign, c)| {
                ...
                (from_basis_string::<W>(&key, n_units), coeff)
            })
            .collect::<Vec<_>>()
    });
    let expectation_value = diag_state.map_or(0.0, |f| op.expectation::<PauliAlgebra>(f));
