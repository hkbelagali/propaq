...
Reads fcidump_N2.txt, constructs the JW Hamiltonian in the spin-orbital basis,
and saves it to hamiltonian_cache.npz with keys: paulis, coeffs.

Qubit ordering: ffsim blocked convention — qubits 0..norb-1 are alpha,
qubits norb..2*norb-1 are beta (matching the UCJ circuit layout).
"""
...
# Expand 1-body integrals to spin-orbital basis (OpenFermion interleaved: even=alpha, odd=beta)
h1e_so = np.zeros((n_qubits, n_qubits))
h1e_so[0::2, 0::2] = h1e  # alpha-alpha
h1e_so[1::2, 1::2] = h1e  # beta-beta

# Expand 2-body integrals: chemist (pq|rs) → physicist <pq|rs>, then spin-orbital blocks
...
# OpenFermion convention: H_2 = Σ h2[p,q,r,s] a†_p a†_q a_r a_s
# Physical:              H_2 = (1/2) Σ <pq|rs> a†_p a†_q a_s a_r → h2[p,q,r,s] = 0.5*<pq|sr>
molecular_hamiltonian = of.InteractionOperator(
    constant=constant,
    one_body_tensor=h1e_so,
    two_body_tensor=0.5 * h2e_so.transpose(0, 1, 3, 2),
)
qubit_op = of.jordan_wigner(of.get_fermion_operator(molecular_hamiltonian))

# Remap OpenFermion's interleaved qubit ordering to ffsim's blocked ordering.
# OF interleaved: qubit 2i = alpha MO i, qubit 2i+1 = beta MO i
# ffsim blocked:  qubit i  = alpha MO i, qubit num_orb+i = beta MO i
